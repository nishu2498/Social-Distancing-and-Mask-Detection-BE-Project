...
class main:
    ...
    def registration(self):
        root.destroy()
        from subprocess import call
        call(["python", "lecture_registration.py"])
 ...
 # , relwidth=1, relheight=1)
    ...
    # Draw Widgets
    def widgets(self):
        self.head = tk.Label(self.master, text='Welcome To Login', background="grey", font=('Times New Roman', 20),
                             pady=20)
        self.head.pack()
        ...
        # self.head.pack()
        self.logf = tk.Frame(self.master, padx=50, pady=20, background="grey")
        tk.Label(self.logf, text='Username: ', background="grey", font=("Times New Roman", 20), pady=5,
                 padx=5).grid(sticky=tk.W)
        tk.Entry(self.logf, textvariable=self.username, bd=5, background="white", font=('', 15)).grid(row=0, column=1)
        tk.Label(self.logf, text='Password: ', background="grey", font=("Times New Roman", 20), pady=5,
                 padx=5).grid(sticky=tk.W)
